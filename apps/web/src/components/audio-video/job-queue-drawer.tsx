    ...
    <>
      ...
      <div className="fixed bottom-5 right-5 z-40">
        <button
          ...
          className="flex items-center gap-2.5 px-4 py-2 rounded-full bg-slate-900/90 border border-white/15 backdrop-blur-md shadow-2xl hover:border-pink-500/50 transition-all duration-200 group text-xs text-white"
        >
          <div className="relative">
            <Icon name="settings" size={14} className={activeJobs.length > 0 ? "animate-spin text-pink-400" : "text-slate-400"} />
            {activeJobs.length > 0 && (
              <span className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-pink-500 animate-ping" />
            )}
          </div>
          <span className="font-semibold">
            {activeJobs.length > 0 ? `${activeJobs.length} Worker Tasks Active` : "Job Queue"}
          </span>
        </button>
      </div>
      ...
      {isDrawerOpen && (
        <div className="fixed bottom-16 right-5 z-50 w-full max-w-sm animate-fade-in">
          <GlassCard className="p-4 border-white/10 shadow-2xl relative flex flex-col max-h-[460px] overflow-hidden" glow>
            ...
            <div className="flex items-center justify-between pb-3 mb-3 border-b border-white/10 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-bold text-white uppercase tracking-wider">
                  Asynchronous Tasks ({jobs.length})
                </h4>
              </div>
              <div className="flex items-center gap-1">
                <button
                  onClick={clearCompletedJobs}
                  className="text-[10px] text-slate-400 hover:text-white px-2 py-0.5 rounded hover:bg-white/5"
                >
                  Clear Finished
                </button>
                <button
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
              {jobs.length === 0 ? (
                <p className="text-xs text-slate-500 text-center py-6 italic">No background tasks queued.</p>
              ) : (
                jobs.map((job) => {
                  ...
                  return (
                    <div
                      key={job.id}
                      className="p-3 bg-white/[0.02] border border-white/5 rounded-xl space-y-2 text-xs"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-bold text-white truncate max-w-[200px]">{job.outputName}</p>
                          <p className="text-[10px] text-slate-400">Source: {job.assetName}</p>
                        </div>
                        <span
                      ...
                      {isFinished && job.outputSizeBytes && (
                        <p className="text-[10px] text-emerald-400">
                          ✓ Completed ({formatBytes(job.outputSizeBytes)})
                        </p>
                      )}
